...
Dins de l'arrel del projecte creem el fitxer docker-compose.yml -> sudo docker-compose.yml
...
També podem obrir el docker fent a l'arrel del projecte -> sudo docker-compose up -d

=== Configuració de servidor web:

Si no el tenim ja posem el nostre projecte a /var/www/html
...
<VirtualHost *:80>
    ...
    DocumentRoot /var/www/html/elteuprojecte/public

    <Directory /var/www/html/elteuprojecte>
...
Hem de donar permisos a la carpeta de logs i framework amb aquestes dos comandes -> chmod -R 775 /var/www/html/elteuprojecte/storage/framework /var/www/html/elteuprojecte/storage/logs

I hem de fer un link simbòlic entre storage/app i public amb -> php artisan storage:link
