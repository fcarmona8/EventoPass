...
* `name`: Nom de la categoria, varchar.

=== Taula: Events
* `id`: Clau primària, bigint.
* `name`: Nom de l'esdeveniment, varchar.
...
* `main_image_id`: Clau forana a EventImages, bigint.
* `event_date`: Data de l'esdeveniment, timestamp.
* `max_capacity``: Capacitat màxima, integer.
* `video_link`: Enllaç a vídeo promocional, varchar.
* `hidden`: Indica si l'esdeveniment està ocult, boolean.
* `nominal`: Indica si l'esdeveniment és nominal, boolean.
* `user_id`: Clau forana a Users, bigint.

=== Taula: Venues
* `id`: Clau primària, bigint.
* `province`: Província, varchar.
* `city`: Ciutat, varchar.
* `postal_code`: Codi postal, varchar.
* `venue_name`: Nom del recinte, varchar.
* `capacity`: Capacitat, integer.
* `user_id`: Clau forana a Users, bigint.

=== Taula: Users
* `id`: Clau primària, bigint.
* `name`: Nom de l'usuari, varchar.
* `email`: Email de l'usuari, varchar.
* `email_verified_at`: Data de verificació de l'email, timestamp.
* `password`: Contrasenya, varchar.
* `remember_token`: Token de recordatori, varchar.
* `role_id`: Clau forana a Roles, bigint.

=== Taula: Roles
* `id`: Clau primària, bigint.
* `name`: Nom del rol, varchar.

=== Taula: EventImages
* `id`: Clau primària, bigint.
* `event_id`: Clau forana a Events, bigint.
* `image_url`: URL de la imatge, varchar.
* `is_main`: Indica si és la imatge principal, boolean.

=== Taula: Sessions
* `id`: Clau primària, bigint.
* `event_id`: Clau forana a Events, bigint.
* `date_time`: Data i hora de la sessió, timestamp.
* `online_sale_end_time`: Data i hora de fi de venda en línia, timestamp.
* `max_capacity`: Capacitat màxima, integer.
* `named_tickets`: Indica si els tiquets són nominals, boolean.
* `closed`: Indica si la sessió està tancada, boolean.

=== Taula: TicketTypes
* `id`: Clau primària, bigint.
* `name`: Nom del tipus de tiquet, varchar.
* `price`: Preu, numeric(8,2).
* `available_tickets`: Entrades disponibles, integer.
...
* `type_id`: Clau forana a TicketTypes, bigint.
* `session_id`: Clau forana a Sessions, bigint.
* `is_validated`: Indica si el tiquet està validat, boolean.
* `name`: Nom de l'assistent, varchar.
* `dni`: DNI de l'assistent, varchar.
* `telefono`: Telèfon de l'assistent, varchar.
* `unicIdTicket`: ID únic del tiquet, varchar.
* `buyerName`: Nom del comprador, varchar.

=== Taula: Comentaris
* `id`: Clau primària, bigint.
* `nombre`: Nom de l'usuari que realitza el comentari, varchar.
* `event_id`: Clau forana que referència a Events, bigint.
* `smileyRating`: Qualificació del comentari en forma d'emoticones, int, nullable.
* `puntuacion`: Puntuació numèrica del comentari, int, nullable.
* `titulo`: Títol del comentari, varchar.
* `comentario`: Text del comentari, text.
* `created_at`: Data i hora en què es va crear el comentari, timestamp.
* `updated_at`: Data i hora en què es va actualitzar el comentari, timestamp.
